...
import { ExpandHint, ImageLightbox, useImagePreview } from "@/components/image-lightbox";
...
export function BillReceipt({ bill, items, landlord, tenant, payments, viewer }: BillReceiptProps) {
  ...
  // Shared lightbox for the meter photo and any payment proof images. We use
  // a single instance so the user can tap any image inside the receipt to
  // expand it full-screen with aspect-ratio preserved (resizeMode contain).
  ...
  return (
    ...
    >
      ...
      {bill.meterPhotoUrl ? (
        <>
          ...
          <View>
            ...
            <Pressable
              onPress={() => preview.open(resolveUrl(bill.meterPhotoUrl))}
              accessibilityRole="imagebutton"
              accessibilityLabel="View meter photo full screen"
            ...
            {payments.map((p) => {
              ...
              return (
                <View key={p.id} style={{ marginBottom: 10 }}>
                  <Pressable
                    onPress={() => preview.open(resolveUrl(p.proofUrl))}
                    accessibilityRole="imagebutton"
                    accessibilityLabel="View payment proof full screen"
                    style={({ pressed }) => [{ opacity: pressed ? 0.85 : 1 }]}
                  >
                    <Image
      ...
      {/* Full-screen image preview triggered by any tappable thumbnail above. */}
